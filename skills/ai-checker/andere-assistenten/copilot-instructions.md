# betteryields AI-Check -- instructions for GitHub Copilot

Copy this file to `.github/copilot-instructions.md` in the repository you work in,
and put `betteryields-ai-check.py` beside it, in `.github/`. Copilot reads the file
on its own in every chat of that repository. Agent mode may run the script; ask mode
may not, and then it says so instead of guessing.
...
Use it whenever you are asked to summarise, review, compare, audit or extract from a
`.docx`, `.pdf`, `.xlsx` or `.pptx` in this repository and the answer has to be
trustworthy, and whenever you are asked to check an answer another AI produced.

The script needs Python 3.11 or newer and nothing else. On Windows the command is
`python`, not `python3`.

## The one rule that cannot be bent

**Never write, paraphrase, complete or summarise a report you did not receive from
the script.** If you cannot run it -- no terminal in this mode, no Python -- say so
plainly and stop. A fabricated report is worse than no check at all: it is the exact
failure this tool exists to catch, and the person reading it has no way to tell.

Show the report as the script printed it, in a code block. It carries the checksums.

## How to run a check

1. **Pick the mode** from the table below.
2. **Get the instruction from the script, do not write it yourself.** In the terminal:
   `python3 .github/betteryields-ai-check.py prompt <mode>`
   It prints the exact format the checker parses. A copy in your own words drifts
   away from the checker on its next release.
3. **Write your answer in that format** and save it as a UTF-8 text file in the
   workspace.
4. **Run the check:**
   `python3 .github/betteryields-ai-check.py <answer.txt> <document> [second document]`
...
`python3 .github/betteryields-ai-check.py generate questions <document>`,
